""" Modified by Chang Liu <changliu@microsoft.com>
    <https://github.com/riannevdberg/sylvester-flows/blob/master/models/layers.py>
"""
import torch
...
class GatedConv2d(nn.Module):
    ...
    def forward(self, x, eval_jac=False):
...
class MaskedLinear(nn.Module):
    ...
    def forward(self, x, eval_jac=False):
        ...
        if eval_jac:
            if self.bias is not None:
                return output.add(self.bias.expand_as(output)), self.mask * self.weight
            else:
                return output, self.mask * self.weight
        else:
            if self.bias is not None:
